...
class Snake:
    ...
    def check_die(self, x_min, x_max, y_min, y_max):
        ...
        for piece in self.pieces:
            if piece.x < x_min or piece.x > x_max or piece.y < y_min or piece.y > y_max:
                self.alive = False
...
def c_snake(l):
    return Snake(l, grid_margin + (grid_width - math.ceil(grid_width/2)) * (grid_scl + grid_margin),
                 grid_margin + (grid_height - math.ceil(grid_height/2)) * (grid_scl + grid_margin), Snake.Direction.RIGHT)
